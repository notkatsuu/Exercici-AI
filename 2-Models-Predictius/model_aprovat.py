...
si un alumne aprovarà (G3 ≥ 10) utilitzant diversos algorismes.
"""
...
def analyze_target_distribution(data: pd.DataFrame, target_column: str = 'G3') -> None:
    """
    ...
    binary_target = (target >= 10).astype(int)  # Aprovat: G3 >= 10
    ...
    # Distribució de valors binaris (aprovat/suspès)
    value_counts = binary_target.value_counts()
    total = len(binary_target)
    logger.info(f"Distribució d'aprovats/suspesos:")
    logger.info(f"  Suspesos (0): {value_counts.get(0, 0)} estudiants ({100 * value_counts.get(0, 0) / total:.2f}%)")
    logger.info(f"  Aprovats (1): {value_counts.get(1, 0)} estudiants ({100 * value_counts.get(1, 0) / total:.2f}%)")
...
def train_classification_model(X: pd.DataFrame, y: pd.Series,
    ...
    """
    logger.info(f"Entrenant model de classificació {model_type} per predir aprovats")
    ...
    model = Pipeline(steps=[
        ...
        ('feature_selection', SelectFromModel(estimator=RandomForestClassifier(n_estimators=100, random_state=RANDOM_STATE))),
...
def evaluate_classification_model(model: Pipeline, X_test: pd.DataFrame,
    ...
    logger.info(f"Mètriques de classificació:")
...
def plot_feature_importance(model: Pipeline, column_names: List[str],
    ...
    try:
        ...
        if hasattr(classifier, 'feature_importances_'):
            ...
            # Si el pipeline té selecció de característiques, necessitem remapejar les importàncies
            if 'feature_selection' in model.named_steps:
                ...
                if hasattr(preprocessor, 'get_feature_names_out'):
                    ...
                    feature_names = [f'feature_{i}' for i in range(len(selected_indices))]
                ...
                # Cas sense selecció de característiques
                preprocessor = model.named_steps['preprocessor']
                feature_names = preprocessor.get_feature_names_out()
                feature_importances_dict = {feature: importance
            ...
            sorted_indices = np.argsort(list(feature_importances_dict.values()))
            sorted_features = [list(feature_importances_dict.keys())[i] for i in sorted_indices[-20:]]  # Top 20
            sorted_importances = [feature_importances_dict[feature] for feature in sorted_features]

            plt.figure(figsize=(10, 8))
            plt.barh(range(len(sorted_importances)), sorted_importances, align='center')
            plt.yticks(range(len(sorted_importances)), [str(f) for f in sorted_features])
            plt.title('Importància de les Característiques en la Predicció d\'Aprovats')
...
def plot_roc_pr_curves(metrics: Dict[str, Union[float, np.ndarray]], output_path: str) -> None:
    ...
    ax2.set_title('Corba Precision-Recall')
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description='Model de predicció d\'aprovats'
    )
    ...
    model, params = train_classification_model(
        X_train, y_train, preprocessor, model_type=args.model_type
    )
    ...
    model_path = os.path.join(args.output_dir, "decision_tree_pass.joblib")
    params_path = os.path.join(args.output_dir, "params_pass.json")
    save_model_and_params(model, params, model_path, params_path)

    # Genera i guarda gràfics
    # Gràfic d'importància de característiques
    feature_importance_path = os.path.join(args.output_dir, "feature_importance_pass.png")
    plot_feature_importance(model, list(X.columns), feature_importance_path)

    # Gràfic de matriu de confusió
    confusion_matrix_path = os.path.join(args.output_dir, "confusion_matrix_pass.png")
    plot_confusion_matrix(metrics['confusion_matrix'], confusion_matrix_path)

    # Gràfic de corbes ROC i PR
    if 'roc_auc' in metrics:
        curves_path = os.path.join(args.output_dir, "roc_pr_curves_pass.png")
        plot_roc_pr_curves(metrics, curves_path)

    # Mostra un resum final
    logger.info("El model d'aprovats ha estat entrenat i avaluat correctament.")
    print("\n" + "="*50)
    print("Model de predicció d'aprovats")
    print("="*50)
    ...
    print("\nFitxers generats:")
    print(f"- {os.path.basename(model_path)} - Model de classificació per aprovat/suspès")
