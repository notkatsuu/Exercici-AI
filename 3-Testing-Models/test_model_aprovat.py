...
# Columnes esperades pel model d'aprovats (totes menys 'G3')
EXPECTED_COLUMNS_PASS = [
...
class PassPredictorApp:
    def __init__(self, master):
        self.master = master
        master.title("Predicció d'Aprovats")
        ...
        self.model_path = '../2-Models-Predictius/decision_tree_pass.joblib'
        self.model = self.load_model()

        self.entries = {}
        self.create_input_fields()

        self.predict_button = ttk.Button(master, text="Predir Aprovat", command=self.predict_pass)
    ...
    def load_model(self):
        try:
            ...
            print(f"Model d'aprovats carregat des de: {self.model_path}")
            return model
    ...
    def create_input_fields(self):
        ...
        for i, col_name in enumerate(EXPECTED_COLUMNS_PASS):
            definition = COLUMN_DEFINITIONS_PASS.get(col_name)
            if not definition:
                continue
    ...
    def predict_pass(self):
        ...
        try:
            for col_name, entry_widget in self.entries.items():
                ...
                definition = COLUMN_DEFINITIONS_PASS[col_name]
                ...
                if expected_type == int:
                    val_int = int(val_str)
                    # Add specific validations for pass model if different from absences
            ...
            df_to_predict = df_to_predict[EXPECTED_COLUMNS_PASS]

            prediction = self.model.predict(df_to_predict)
            probabilities = self.model.predict_proba(df_to_predict)
            
            result_text = f"Predicció: {'Aprovarà' if prediction[0] == 1 else 'Suspendrà'}\n"
            result_text += f"Probabilitats: [Suspendrà: {probabilities[0][0]:.2f}, Aprovarà: {probabilities[0][1]:.2f}]"
