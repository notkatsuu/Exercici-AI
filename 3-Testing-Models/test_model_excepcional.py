...
# Columnes esperades pel model d'excepcionals (totes menys 'G3')
EXPECTED_COLUMNS_EXCEPTIONAL = [
...
# Són les mateixes definicions que per al model d'aprovats
COLUMN_DEFINITIONS_EXCEPTIONAL = {
    ...
    'Medu': {"prompt": "Educ. mare (0-4):", "type": int, "default": 4}, # Default més alt per exemple
    'Fedu': {"prompt": "Educ. pare (0-4):", "type": int, "default": 4}, # Default més alt per exemple
    'Mjob': {"prompt": "Feina mare:", "type": str, "options": ['teacher', 'health', 'services', 'at_home', 'other'], "default": 'teacher'},
    'Fjob': {"prompt": "Feina pare:", "type": str, "options": ['teacher', 'health', 'services', 'at_home', 'other'], "default": 'teacher'},
    'reason': {"prompt": "Raó escola:", "type": str, "options": ['home', 'reputation', 'course', 'other'], "default": 'reputation'},
    'guardian': {"prompt": "Tutor:", "type": str, "options": ['mother', 'father', 'other'], "default": 'mother'},
    'traveltime': {"prompt": "Temps viatge (1-4):", "type": int, "default": 1},
    'studytime': {"prompt": "Temps estudi (1-4):", "type": int, "default": 3}, # Default més alt
    ...
    'higher': {"prompt": "Vol estudis sup. (yes/no):", "type": str, "options": ['yes', 'no'], "default": 'yes'},
    ...
    'famrel': {"prompt": "Rel. familiar (1-5):", "type": int, "default": 5},
    'freetime': {"prompt": "Temps lliure (1-5):", "type": int, "default": 3},
    'goout': {"prompt": "Sortides (1-5):", "type": int, "default": 2},
    'Dalc': {"prompt": "Alcohol lab. (1-5):", "type": int, "default": 1},
    'Walc': {"prompt": "Alcohol cap set. (1-5):", "type": int, "default": 1},
    'health': {"prompt": "Salut (1-5):", "type": int, "default": 5},
    'absences': {"prompt": "Absències:", "type": int, "default": 0},
    'G1': {"prompt": "Nota G1 (0-20):", "type": int, "default": 17}, # Default més alt per exemple
    'G2': {"prompt": "Nota G2 (0-20):", "type": int, "default": 17}  # Default més alt per exemple
}

class ExceptionalPredictorApp:
    def __init__(self, master):
        self.master = master
        master.title("Predicció d'Alumnes Excepcionals")
        ...
        self.model_path = '../2-Models-Predictius/decision_tree_exceptional.joblib'
        self.model = self.load_model()

        self.entries = {}
        self.create_input_fields()

        self.predict_button = ttk.Button(master, text="Predir Excepcional", command=self.predict_exceptional)
    ...
    def load_model(self):
        try:
            ...
            print(f"Model d'excepcionals carregat des de: {self.model_path}")
            return model
    ...
    def create_input_fields(self):
        ...
        for i, col_name in enumerate(EXPECTED_COLUMNS_EXCEPTIONAL):
            definition = COLUMN_DEFINITIONS_EXCEPTIONAL.get(col_name)
            if not definition:
                continue
    ...
    def predict_exceptional(self):
        ...
        try:
            for col_name, entry_widget in self.entries.items():
                ...
                definition = COLUMN_DEFINITIONS_EXCEPTIONAL[col_name]
                expected_type = definition["type"]

                if not val_str and definition["default"] is not None:
                    val_str = str(definition["default"])

                if expected_type == int:
                    val_int = int(val_str)
                    # Add specific validations for exceptional model if different
                    ...
                    if col_name in ['G1', 'G2', 'age', 'failures', 'absences'] and val_int < 0:
                        raise ValueError(f"{col_name}: El valor no pot ser negatiu.")
                    if col_name in ['G1', 'G2'] and val_int > 20:
            ...
            df_to_predict = df_to_predict[EXPECTED_COLUMNS_EXCEPTIONAL]

            prediction = self.model.predict(df_to_predict)
            probabilities = self.model.predict_proba(df_to_predict)
            
            # Recorda que el llindar per excepcional es va canviar a >=17 en pac3_models.py
            result_text = f"Predicció: {'Excepcional (G3>=17)' if prediction[0] == 1 else 'No excepcional (G3<17)'}\n"
            result_text += f"Probabilitats: [No Excepcional: {probabilities[0][0]:.2f}, Excepcional: {probabilities[0][1]:.2f}]"
...
if __name__ == '__main__':
    root = tk.Tk()
    app = ExceptionalPredictorApp(root)
    root.mainloop()
